
extern const unsigned char IMAGE_DATA[];
extern const unsigned char IMAGE_ONNETWORK[];
extern const unsigned char IMAGE_OFFNETWORK[];
extern const unsigned char IMAGE_TEMPERATURE[];
extern const unsigned char IMAGE_HUMIDITY[];
extern const unsigned char IMAGE_PRESSURE[];
extern const unsigned char IMAGE_ILLUMINANCE[];

extern const unsigned char IMAGE_MOTION[];
extern const unsigned char IMAGE_MOTION_NOT[];

extern const unsigned char IMAGE_BATTERY_100[];
extern const unsigned char IMAGE_BATTERY_75[];
extern const unsigned char IMAGE_BATTERY_50[];
extern const unsigned char IMAGE_BATTERY_25[];
extern const unsigned char IMAGE_BATTERY_0[];

extern const unsigned char IMAGE_DOWN[];
extern const unsigned char IMAGE_UP[];
extern const unsigned char IMAGE_CLEAR[];
extern const unsigned char IMAGE_RIGHT[];
extern const unsigned char IMAGE_LEFT[];
extern const unsigned char IMAGE_CLEAR16[];

extern const unsigned char IMAGE_LQI_100[];
extern const unsigned char IMAGE_LQI_80[];
extern const unsigned char IMAGE_LQI_60[];
extern const unsigned char IMAGE_LQI_40[];
extern const unsigned char IMAGE_LQI_20[];
extern const unsigned char IMAGE_LQI_0[];